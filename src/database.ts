...
async function updateUsernameIndex(collection: Collection, unique: boolean) {
	...
	if (!hasIndex) {
		...
		logger.info(
			`Created username index for collection ${collection.collectionName}`,
		);
	}
}
...
export async function initDB() {
	if (ServerConfig.database?.uri && ServerConfig.database?.databaseName) {
		try {
			...
			logger.info(
				`Connected to ${client.options.dbName} on ${client.options.hosts} (Authenticated: ${client.options.credentials ? "Yes" : "No"})`,
			);
		} catch (err) {
			logger.debug(err);
...
async function handleGetPreferences(
	...
	try {
		const doc = await preferenceCollection.findOne(
...
async function handleSetPreferences(
	...
	if (!validUpdate) {
		logger.warning(
			`Rejecting invalid preference update:\n${validatePreferences.errors}`,
		);
		return next({ statusCode: 400, message: "Invalid preference update" });
	}

	try {
		const updateResult = await preferenceCollection.updateOne(
			{ username: req.username },
			{ $set: update },
			{ upsert: true },
		);
...
async function handleGetLayouts(
	...
	try {
		const layoutList = await layoutsCollection
			.find({ username: req.username }, { projection: { _id: 0, username: 0 } })
			.toArray();
		...
		for (const entry of layoutList) {
			if (entry.name && entry.layout) {
				...
				if (!isValid) {
					logger.warning(
						`Returning invalid layout '${entry.name}':\n${validateLayout.errors}`,
					);
				}
...
async function handleSetLayout(
	...
	if (
		!layoutName ||
		!layout ||
		layout.layoutVersion !== LAYOUT_SCHEMA_VERSION
	) {
	...
	if (!validUpdate) {
		logger.warning(
			`Rejecting invalid layout update:\n${validateLayout.errors}`,
		);
		return next({ statusCode: 400, message: "Invalid layout update" });
	}

	try {
		const updateResult = await layoutsCollection.updateOne(
			{ username: req.username, name: layoutName, layout },
			{ $set: { layout } },
			{ upsert: true },
		);
		if (updateResult.acknowledged) {
...
async function handleGetSnippets(
	req: AuthenticatedRequest,
	res: Response,
	next: NextFunction,
) {
	...
	try {
		const snippetList = await snippetsCollection
			.find({ username: req.username }, { projection: { _id: 0, username: 0 } })
			.toArray();
		...
		for (const entry of snippetList) {
			if (entry.name && entry.snippet) {
				...
				if (!isValid) {
					logger.warning(
						`Returning invalid snippet '${entry.name}':\n${validateSnippet.errors}`,
					);
				}
...
async function handleSetSnippet(
	...
	if (
		!snippetName ||
		!snippet ||
		snippet.snippetVersion !== SNIPPET_SCHEMA_VERSION
	) {
	...
	if (!validUpdate) {
		logger.error(
			`Rejecting invalid snippet update:\n${validateSnippet.errors}`,
		);
		return next({ statusCode: 400, message: "Invalid snippet update" });
	}

	try {
		const updateResult = await snippetsCollection.updateOne(
			{ username: req.username, name: snippetName, snippet },
			{ $set: { snippet } },
			{ upsert: true },
		);
		if (updateResult.acknowledged) {
...
async function handleClearSnippet(
	req: AuthenticatedRequest,
	res: Response,
	next: NextFunction,
) {
...
async function handleGetWorkspaceList(
	req: AuthenticatedRequest,
	res: Response,
	next: NextFunction,
) {
	...
	try {
		const workspaceList = await workspacesCollection
			.find(
				{ username: req.username },
				{ projection: { _id: 1, name: 1, "workspace.date": 1 } },
			)
			.toArray();
		const workspaces =
			workspaceList?.map((w) => ({
...
async function handleGetWorkspaceByName(
	req: AuthenticatedRequest,
	res: Response,
	next: NextFunction,
) {
	...
	try {
		const queryResult = await workspacesCollection.findOne(
			{ username: req.username, name: req.params.name },
		...
		if (!queryResult?.workspace) {
			...
			if (!isValid) {
				logger.warning(
					`Returning invalid workspace '${workspace.name}':\n${validateWorkspace.errors}`,
				);
			}
...
async function handleGetWorkspaceByKey(
	req: AuthenticatedRequest,
	res: Response,
	next: NextFunction,
) {
...
async function handleSetWorkspace(
	...
	if (
		!workspaceName ||
		!workspace ||
		workspace.workspaceVersion !== WORKSPACE_SCHEMA_VERSION
	) {
	...
	if (!validUpdate) {
		logger.error(
			`Rejecting invalid workspace update:\n${validateWorkspace.errors}`,
		);
	...
	try {
		const updateResult = await workspacesCollection.findOneAndUpdate(
			{ username: req.username, name: workspaceName },
			{ $set: { workspace } },
...
databaseRouter.get(
	...
	authGuard,
	noCache,
	handleGetWorkspaceByKey,
);
databaseRouter.get(
	"/workspace/:name",
	authGuard,
	noCache,
	handleGetWorkspaceByName,
);
