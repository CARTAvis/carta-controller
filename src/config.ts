...
try {
	...
	if (fs.existsSync(configDir)) {
		...
		for (const file of files) {
			...
			const additionalConfig: any = JSONC.parse(
				jsonString,
			) as CartaServerConfig;
	...
	if ("logFileTemplate" in serverConfig) {
		logger.warning(
			"The 'logFileTemplate' option is deprecated and renamed to 'backendLogFileTemplate'. Please update your config file.",
		);
		if (
			!serverConfig.backendLogFileTemplate ||
			serverConfig.backendLogFileTemplate === ""
		) {
			serverConfig.backendLogFileTemplate = String(
				serverConfig.logFileTemplate,
			);
		} else if (
			serverConfig.backendLogFileTemplate !== serverConfig.logFileTemplate
		) {
			logger.error(
				"'logFileTemplate' and 'backendLogFileTemplate' are both set, and have conflicting values. Ignoring 'logFileTemplate'.",
			);
		}
	...
	if (serverConfig.timezone) {
		try {
			...
			logger.error(
				`Ignoring invalid timezone "${serverConfig.timezone}" in config file`,
			);
		}
	}
	...
	consoleTransport.format =
		serverConfig.logTypeConsole === "json" ? logJsonFormat : logColorTextFormat;
	...
	if (serverConfig.logFile && serverConfig.logFile !== "") {
		if (serverConfig.logLevelFile === "none") {
			logger.error(
				`Log file "${serverConfig.logFile}" specified but with a log level of "none"`,
			);
		} else {
			try {
				logger.add(
...
if (serverConfig.authProviders.external) {
	runtimeConfig.tokenRefreshAddress =
		serverConfig.authProviders.external.tokenRefreshAddress;
	runtimeConfig.logoutAddress =
		serverConfig.authProviders.external.logoutAddress;
} else {
	runtimeConfig.tokenRefreshAddress =
		runtimeConfig.apiAddress + "/auth/refresh";
