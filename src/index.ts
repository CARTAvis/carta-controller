...
if (testUser) {
	...
	if (ServerConfig.frontendPath) {
		...
		app.use(
			"/",
			express.static(
				path.join(__dirname, "../node_modules/carta-frontend/build"),
				{ setHeaders: staticHeaderHandler },
			),
		);
	}
	...
	if (ServerConfig.dashboard?.bannerImage) {
		const isBannerSvg = ServerConfig.dashboard.bannerImage
			.toLowerCase()
			.endsWith(".svg");
		const bannerDataBase64 = fs.readFileSync(
			ServerConfig.dashboard.bannerImage,
			"base64",
		);
		if (isBannerSvg) {
	...
	app.get("/frontend", (req, res) => {
		...
		if (queryString) {
			return res.redirect(
				(ServerConfig.serverAddress ?? "") + "/?" + queryString,
			);
		} else {
	...
	app.post(
		"/api/scripting/*",
		authGuard,
		createScriptingProxyHandler(backendProxy),
	);
	...
	async function init() {
		...
		const onListenStart = () => {
			logger.info(
				`Started listening for login requests on port ${ServerConfig.serverPort}`,
			);
		};
	...
	init().then(() =>
		logger.info(
			`Server initialised successfully at ${ServerConfig.serverAddress ?? "localhost"}`,
		),
	);
}
