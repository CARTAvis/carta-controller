...
export function getLdapLoginHandler(authConf: CartaLdapAuthConfig) {
	...
	return (req: express.Request, res: express.Response) => {
		...
		if (!username || !password) {
			return res
				.status(400)
				.json({ statusCode: 400, message: "Malformed login request" });
		}
		...
		const handleAuth = (err: Error | string, user: any) => {
			...
			if (user?.uid !== username) {
				logger.warning(
					`Returned user "uid ${user?.uid}" does not match username "${username}"`,
				);
				logger.debug(user);
			}
			try {
				...
				logger.info(
					`Authenticated as user ${username} with uid ${uid} using LDAP`,
				);
				...
				return res
					.status(403)
					.json({ statusCode: 403, message: "User does not exist" });
			}
		};
		...
		ldap.authenticate(username, password, (error, user) => {
			...
			if (errorObj?.name?.includes("ConfidentialityRequiredError")) {
				logger.warning(
					`TLS error encountered. Reconnecting to the LDAP server!`,
				);
				ldap.close();
				ldap = new LdapAuth(authConf.ldapOptions);
				ldap.on("error", (err) => logger.error("LdapAuth: ", err));
