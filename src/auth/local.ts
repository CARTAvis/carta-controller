...
export function generateLocalVerifier(
		...
		if (refreshTokenCookie) {
			try {
				...
				if (!refreshToken || !refreshToken.username || !refreshToken.refresh) {
					...
					logger.info(
						`Refreshed ${scriptingToken ? "scripting" : "access"} token for user ${refreshToken.username} with uid ${uid}`,
					);
					res.json({
						access_token,
						token_type: "bearer",
