...
export async function googleCallbackHandler(
	req: Request,
	res: Response,
	authConf: CartaGoogleAuthConfig,
) {
	...
	if (
		...
		return res
			.status(400)
			.json({ error: "Missing or non-matching CSRF token" });
	}
	...
	try {
		...
		if (!username || !payload?.email_verified) {
			logger.warning(
				"Google auth rejected due to lack of unique ID or email verification",
			);
			return res
				.status(500)
				.json({ error: "An error occured processing your login" });
		}

		// check that domain is valid
		if (authConf.validDomain && authConf.validDomain !== payload.hd) {
			logger.warning(
				`Google auth rejected due to incorrect domain: ${payload.hd}`,
			);
			return res
				.status(500)
				.json({ error: "An error occured processing your login" });
		...
		return res.redirect(
			`${RuntimeConfig.dashboardAddress}?googleuser=${username}`,
		);
	} catch (e) {
		logger.debug(e);
		return res
		...
		if (refreshTokenCookie) {
			try {
				...
				if (!refreshToken || !refreshToken.username || !refreshToken.refresh) {
					...
					scriptingToken &&
					ServerConfig.scriptingAccess !== ScriptingAccess.Enabled
				) {
					next({
						statusCode: 500,
					...
					const access_token = generateToken(
						...
						scriptingToken ? TokenType.Scripting : TokenType.Access,
					);
					logger.info(
						`Refreshed ${scriptingToken ? "scripting" : "access"} token for user ${refreshToken.username}`,
					);
					res.json({
						access_token,
						token_type: "bearer",
						...
						expires_in:
							ms(
								scriptingToken
									? authConf.scriptingTokenAge
									: (authConf.accessTokenAge as string),
							) / 1000,
					});
				}
			} catch (err) {
