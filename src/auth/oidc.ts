...
import {
	...
	setRefreshToken,
	getRefreshToken,
} from "./oidcRefreshManager";

let privateKey: KeyObject;
let publicKey: KeyObject;
let symmetricKey: KeyObject;
let jwksManager: GetKeyFunction<
...
export async function initOidc(authConf: CartaOidcAuthConfig) {
	...
	privateKey = createPrivateKey(
		fs.readFileSync(authConf.localPrivateKeyLocation),
	);
	symmetricKey = createSecretKey(
		Buffer.from(
			fs.readFileSync(authConf.symmetricKeyLocation, "utf-8"),
			"base64",
		),
	);

	// Parse details of IdP from metadata URL
	const idpConfig = await axios.get(
		authConf.idpUrl + "/.well-known/openid-configuration",
	);
...
function returnErrorMsg(
	...
	if (req.header("accept") == "application/json") {
		return res
			.status(statusCode)
			.json({ statusCode: statusCode, message: msg });
	} else {
...
async function callIdpTokenEndpoint(
	usp: URLSearchParams,
	req: Request,
	res: Response,
	authConf: CartaOidcAuthConfig,
	scriptingToken: boolean = false,
	isLogin: boolean = false,
	sessionId: string,
	sessionEncKey: Buffer | undefined,
) {
	...
	try {
		...
		if (payload.aud != authConf.clientId) {
			return returnErrorMsg(
				...
				res,
				500,
				"Service received an ID token directed to a different service",
			);
		}
		...
		if (result.data["refresh_token"] !== undefined) {
			setRefreshToken(
				...
				sessionEncKey,
				parseInt(result.data["refresh_expires_in"]),
			);
		}

		const refreshExpiry =
			result.data["refresh_expires_in"] !== undefined
		...
		if (result.data["expires_in"] !== undefined) {
			setAccessTokenExpiry(
				username,
				sessionId,
				parseInt(result.data["expires_in"]),
			);
		}
		...
		if (authConf.requiredGroup !== undefined) {
			if (payload[`${authConf.groupsField}`] === undefined) {
				return returnErrorMsg(
					...
					res,
					403,
					"Identity provider did not supply group membership",
				);
			}
			...
			if (Array.isArray(idpGroups)) {
				...
				return returnErrorMsg(
					...
					res,
					403,
					"Invalid group membership info received",
				);
			}
		}
		...
		return returnErrorMsg(
			...
			res,
			500,
			"Error requesting tokens from identity provider",
		);
	}
}
		...
		if (refreshTokenCookie) {
			try {
				...
				try {
					...
					const remainingValidity = await getAccessTokenExpiry(
						payload.username,
						payload.sessionId,
					);
					if (remainingValidity > authConf.cacheAccessTokenMinValidity) {
						...
						const sessionEncKey = Buffer.from(
							`${payload?.sessionEncKey}`,
							"hex",
						);
						usp.set("grant_type", "refresh_token");
						usp.set(
							"refresh_token",
							`${await getRefreshToken(payload.username, payload.sessionId, sessionEncKey)}`,
						);
						return await callIdpTokenEndpoint(
							usp,
							req,
							res,
							authConf,
...
export function generateLocalOidcVerifier(
...
export async function oidcLoginStart(
	req: Request,
	res: Response,
	authConf: CartaOidcAuthConfig,
) {
	try {
		...
		const codeVerifier = Array.from(
			{ length: 64 },
			(_, i) => urlSafeChars[Math.floor(Math.random() * urlSafeChars.length)],
		).join("");
		const encryptedCodeVerifier = await new jose.CompactEncrypt(
		...
		usp.set(
			"redirect_uri",
			new URL(
				RuntimeConfig.apiAddress + "/auth/oidcCallback",
...
export async function oidcCallbackHandler(
	...
	try {
		...
		const decryptedCodeVerifier = await jose.compactDecrypt(
			req.cookies["oidcVerifier"],
			privateKey,
		);
		const codeVerifier = new TextDecoder().decode(
			decryptedCodeVerifier.plaintext,
		);
