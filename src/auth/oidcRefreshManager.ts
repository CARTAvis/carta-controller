...
export async function initRefreshManager() {
	try {
		...
		if (
			!(await db
				.listCollections({ name: "tokenLock" }, { nameOnly: true })
				.hasNext())
		) {
		...
		if (
			!(await db
				.listCollections({ name: "refreshTokens" }, { nameOnly: true })
				.hasNext())
		) {
		...
		if (
			!(await db
				.listCollections({ name: "accessTokenLifetimes" }, { nameOnly: true })
				.hasNext())
		) {
			logger.info("Creating access token's lifetimes collection");
			accessTokenLifeTimesCollection = await db.createCollection(
				"accessTokenLifetimes",
			);
		} else {
			accessTokenLifeTimesCollection = await db.collection(
				"accessTokenLifetimes",
			);
		}
		...
		if (!hasLockExpiryIndex) {
			await lockCollection.createIndex(
				{ expireAt: 1 },
				{ name: "lockExpiry", expireAfterSeconds: 0 },
			);
		...
		for (let coll of [refreshTokenCollection, accessTokenLifeTimesCollection]) {
			...
			if (!hasUserSessionIndex) {
				await coll.createIndex(
					{ username: 1, sessionid: 1 },
					{ name: "userSession", unique: true },
				);
				logger.info(
					`Created username/session index for collection ${coll.collectionName}`,
				);
			}
			...
			if (!hasExpiryIndex) {
				await coll.createIndex(
					{ expireAt: 1 },
					{ name: "expiryIndex", expireAfterSeconds: 0 },
				);
				logger.info(
					`Created index adding TTL for collection ${coll.collectionName}`,
				);
			}
		}
	} catch (err) {
...
export async function setRefreshToken(
	...
	try {
		...
		const encrypted =
			cipher.update(refreshToken, "utf8", "hex") + cipher.final("hex");
