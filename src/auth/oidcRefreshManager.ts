...
export async function clearTokens(username, sessionid) {
  await Promise.all([
    accessTokenLifeTimesCollection.deleteOne({ username, sessionid }).catch(e => logger.error(e)),
    refreshTokenCollection.deleteOne({ username, sessionid }).catch(e => logger.error(e)),
  ]);
}
