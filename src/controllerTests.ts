...
async function testLog(username: string) {
	const logLocation = ServerConfig.backendLogFileTemplate
	...
	try {
		...
		logger.info(
			`${logSymbols.success} Checked log writing for user ${username}`,
		);
	} catch (err) {
		logger.debug(err);
		throw new Error(
			`Could not create log file at ${logLocation} for user ${username}. Please check your config file's backendLogFileTemplate option`,
		);
	}
}
	...
	return new Promise<void>((resolve, reject) => {
		if (authConf) {
			...
			try {
				...
				setTimeout(() => {
					read({ prompt: `Password for user ${username}:`, silent: true }).then(
						(password) => {
							ldap.authenticate(username, password, (error, user) => {
								if (error) {
									...
									reject(
										new Error(
											`Could not authenticate as user ${username}. Please check your config file's ldapOptions section!`,
										),
									);
								} else {
									logger.info(
										`${logSymbols.success} Checked LDAP connection for user ${username}`,
									);
									if (user?.uid !== username) {
										logger.warning(
											`${logSymbols.warning} Returned user "uid ${user?.uid}" does not match username "${username}"`,
										);
										logger.debug(user);
									}
									resolve();
				...
				reject(
					new Error(
						"Cannot create LDAP object. Please check your config file's ldapOptions section!",
					),
				);
			}
		}
					...
					pamAuthenticate(
						...
						(err: Error | string, code: number) => {
							if (err) {
								...
								reject(
									new Error(
										`Could not authenticate as user ${username}. Error code ${code}`,
									),
								);
							} else {
								logger.info(
									`${logSymbols.success} Checked PAM connection for user ${username}`,
								);
								resolve();
							}
						},
...
async function testDatabase() {
	try {
		...
		logger.debug(e);
		throw new Error(
			"Cannot connect to MongoDB. Please check your config file's database section!",
		);
	}
...
function testUid(username: string) {
	...
	logger.info(
		`${logSymbols.success} Verified uid (${uid}) for user ${username}`,
	);
}
...
function testToken(authConf: CartaLocalAuthConfig, username: string) {
	...
	try {
		...
		logger.debug(e);
		throw new Error(
			"Cannot generate access token. Please check your config file's auth section!",
		);
	}
	if (!token) {
		throw new Error(
			"Invalid access token. Please check your config file's auth section!",
		);
	}
	logger.info(
		`${logSymbols.success} Generated access token for user ${username}`,
	);
}

function testFrontend() {
	if (!ServerConfig.frontendPath) {
		ServerConfig.frontendPath = path.join(
			__dirname,
			"../node_modules/carta-frontend/build",
		);
	}
	...
	try {
		indexContents = fs
			.readFileSync(ServerConfig.frontendPath + "/index.html")
			.toString();
	} catch (e) {
		logger.debug(e);
	...
	if (!indexContents) {
		...
		logger.info(
			`${logSymbols.success} Read frontend index.html from ${ServerConfig.frontendPath}`,
		);
	}
}
...
async function testBackendStartup(username: string) {
	...
	if (backendProcess.signalCode) {
		throw new Error(
			`Backend process terminated with code ${backendProcess.signalCode}. Please check your sudoers config, processCommand option and additionalArgs section`,
		);
	} else {
	...
	if (wsConnected) {
		...
		throw new Error(
			"Cannot connect to backend process. Please check your additionalArgs section. If sudo is prompting you for a password, please check your sudoers config",
		);
	}
...
async function testKillScript(username: string, existingProcess: ChildProcess) {
	if (existingProcess.signalCode) {
		throw new Error(
			`Backend process already killed, signal code ${existingProcess.signalCode}`,
		);
	}
	const args = [
		"-u",
		`${username}`,
		ServerConfig.killCommand,
		`${existingProcess.pid}`,
	];
	logger.debug(`running sudo ${args.join(" ")}`);
	...
	if (res.status) {
		throw new Error(
			`Cannot execute kill script (error status ${res.status}. Please check your killCommand option`,
		);
	}
	...
	if (existingProcess.signalCode === "SIGKILL") {
		...
		throw new Error(
			"Failed to kill process. Please check your killCommand option. If sudo is prompting you for a password, please check your sudoers config",
		);
	}
}
